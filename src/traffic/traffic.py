...
def load_data(data_dir):
    ...
    for directory in os.listdir(data_dir):
        for filename in os.listdir(os.path.join(data_dir, directory)):
            image = cv2.imread(os.path.join(data_dir, directory, filename))
...
def get_model():
    ...
    model = tf.keras.models.Sequential([
        # begin with flatten, the accuracy is 0.7888
        # tf.keras.layers.Flatten(input_shape=(IMG_WIDTH, IMG_HEIGHT, 3)),
        
        # if convolution once, the accuracy is 0.9245
        ...
        # if only add one convolution, the accuray is 0.9313 not good
