...
class Sentence():
    ...
    def known_safes(self):
        ...
        """
        return self.cells if len(self.cells) == 0 else None
...
class MinesweeperAI():
        ...
        for i in [-1, 0, 1]:
            for j in [-1, 0, 1]:
                ...
                if i == j == 0:
                    continue
                neighbors.add((i, j))
        mines_count = len(neighbors & self.mines)
        neighbors = neighbors - self.moves_made - self.mines
        if len(neighbors) != 0:
            self.knowledge.append(Sentence(neighbors, count - mines_count))
        self.conclude_from_knowledge()
        self.infer_from_konwledge()
    ...
    def make_random_move(self):
        ...
        for i in range(self.height):
            for j in range(self.width):
                access_move.add((i, j))
        access_move = access_move - self.mines - self.moves_made
        if len(access_move) != 0:
            random_move = list(access_move)[random.randint(0, len(access_move))]
        return random_move

    def conclude_from_knowledge(self):
        knowledeg = copy.deepcopy(self.knowledge)
        for sentence in knowledeg:
            if len(sentence.cells) == 0:
                knowledeg.remove(sentence)
